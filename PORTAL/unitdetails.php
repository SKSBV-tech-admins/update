...
if (isset($_POST['unitsksbv'])) {
    ...
    if ($num==0) {
        echo "<script>
    alert('Madrassa with SKSBV registration number $sksbvregno not found! Please enter a valid SKSBV register number.');
    window.location.href='unitregistrationdashboard.php';
    </script>";
    }else {
        $data=mysqli_query($con, "SELECT * FROM unitregister where sksbvregno='$sksbvregno'");
